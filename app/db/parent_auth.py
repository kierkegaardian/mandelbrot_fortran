...
def verify_parent_pin(pin: str, now_iso_text: str | None = None) -> bool:
    ...
    with managed_connection() as conn:
        ...
        verified = hmac.compare_digest(computed, str(row["pin_hash"]))
        if verified:
            conn.execute(
                """
                UPDATE parent_auth
                ...
                WHERE profile_id = ?
                """,
                (now.isoformat(), int(row["profile_id"])),
            )
        return verified
...
def record_parent_auth_failure(now_iso_text: str) -> str | None:
    ...
    with managed_connection() as conn:
        row = conn.execute(
            "SELECT profile_id, failed_attempts, locked_until FROM parent_auth LIMIT 1",
        ).fetchone()
        if row is None:
            return None
        existing_lock = _parse_iso_utc(str(row["locked_until"])) if row["locked_until"] else None
        if existing_lock is not None and existing_lock > now:
            return existing_lock.isoformat()
