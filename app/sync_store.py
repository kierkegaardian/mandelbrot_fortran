...
from . import db
from .sync_tombstones import record_tombstone_on
    ...
    *,
    ...
    with db.managed_connection() as conn:
        return enqueue_change_on(
            ...
            extra=extra,
            known_sync_id=known_sync_id,
        )


def enqueue_change_on(
    ...
    *,
    ...
    table = _table_for_entity(entity_type)
    sync_id = known_sync_id
    if sync_id is None:
        row = conn.execute(
            f"SELECT sync_id FROM {table} WHERE id = ?", (int(local_id),)
        ).fetchone()
        sync_id = str(row["sync_id"]) if row is not None and row["sync_id"] else None
    if not sync_id:
...
def soft_delete_entity_on(
    ...
    *,
    ...
    """Replace pending writes with one delete and preserve a local tombstone."""
    table = _table_for_entity(entity_type)
    conn.execute(
        """DELETE FROM sync_outbox
        WHERE entity_type = ? AND entity_sync_id = ? AND synced_at IS NULL""",
    ...
    enqueue_change_on(
        ...
        known_sync_id=entity_sync_id,
    )
    conn.execute(
        f"""UPDATE {table} SET sync_deleted = 1, sync_updated_at = ?
        WHERE id = ?""",
        (deleted_at, int(local_id)),
    )
    record_tombstone_on(
        conn,
        entity_type=entity_type,
