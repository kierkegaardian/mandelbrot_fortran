...
from .sync_state import SyncStateWriteError, load_sync_state, save_sync_enabled
...
class FamilySyncSettings:
    ...
    def _toggle_family_sync(self) -> None:
        ...
        except SyncStateWriteError as exc:
            self.sync_enabled_var.set(load_sync_state().sync_enabled)
            messagebox.showerror(FAMILY_SYNC_BETA_TITLE, str(exc))
            self.refresh()
            return
        self.refresh()
    ...
    def _start_family_sync(self) -> None:
        ...
        if state.family_id and not messagebox.askyesno(
            ...
            return
        profile = self._profile_getter()
        result = sync_service.start_family_sync(profile.id if profile else None)
        self.refresh()
        if result.ok:
    ...
    def _join_family_sync(self) -> None:
        ...
        if pairing_token is None:
            return
        profile = self._profile_getter()
        result = sync_service.join_family_sync(
            pairing_token, profile.id if profile else None
        )
        self.refresh()
        if result.ok:
            messagebox.showinfo(FAMILY_SYNC_BETA_TITLE, result.message)
    ...
    def _sync_now(self) -> None:
        if not self._require_parent_profile():
            return
        profile = self._profile_getter()
        result = sync_service.sync_now(profile.id if profile else None)
        self.refresh()
