...
from .skill_graph import SKILL_ORDER, subskills_for
...
class TexasGoal:
    ...
    def quiz_ready(self) -> bool:
        if self.skill is None or self.skill not in SKILL_ORDER:
            return False
        available = set(subskills_for(self.skill))
        return bool(available) and all(item in available for item in self.subskills)
