...
class SummerProgramTests(unittest.TestCase):
    ...
    def test_preview_unit_quiz_plan_uses_algebra_and_geometry_questions(self) -> None:
        ...
        self.assertGreaterEqual(sum(question.skill == "algebra_1" for question in capstone_plan.questions), 3)
        self.assertTrue(any(question.skill != "algebra_1" for question in capstone_plan.questions))
        self.assertTrue(any(question.subskill == "Slope from points" for question in capstone_plan.questions))
        self.assertTrue(any(question.subskill == "Functions" for question in capstone_plan.questions))
        self.assertFalse(any(question.scaffold_steps for question in capstone_plan.questions))
        self.assertGreaterEqual(sum(question.skill == "geometry_area" for question in geometry_capstone_plan.questions), 3)
        self.assertTrue(any(question.skill != "geometry_area" for question in geometry_capstone_plan.questions))
        ...
        self.assertFalse(any(question.scaffold_steps for question in geometry_capstone_plan.questions))
