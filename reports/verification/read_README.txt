...
- Optional family-sync discovery can be supplied with `MANDELQUEST_SYNC_CONFIG=/path/to/sync-config.json`.
- Public builds ship with no sync server configured; machine-local sync config files should stay outside Git.
- To hard-disable internet-linked actions, enable `Enforce offline mode` in Parent settings.
...
- No cloud/LAN sync yet; data is still local to one machine. Family use across the kids' laptop and the parents' laptops will need central/server-backed progress storage.
